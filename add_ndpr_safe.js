...
for (const file of files) {
  ...
  // Inject before the VERY LAST </body>
  const lastIndex = content.lastIndexOf('</body>');
  if (lastIndex !== -1) {
      content = content.substring(0, lastIndex) + snippet + '\n</body>' + content.substring(lastIndex + 7);
  } else {
      content += snippet;
  }
...
console.log('NDPR Banner injected safely.');
